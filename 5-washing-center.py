...
async def CoroWashingMachine(w, client):
    ...
    while True:
        ...
        if w.STATE == S_SPIN:
            ...
            w.STATE = S_SPIN
            await actionWithinTime(w, client, nextstate=S_FAULT, msg='FAULT')
        # When washing is in FAULT state, wait until get FAULTCLEARED
        if w.STATE == S_FAULT:
            await publish_message(w, client, "app", "get", "STATUS", w.STATE)

            print(f'{time.ctime()} - [{w.SERIAL}-{w.STATE}] FAULT...')
            w.STATE = S_FAULT
            await actionWithinTime(w, client, nextstate=S_FAULTCLEARED, msg='FAULT_CLEARED')
